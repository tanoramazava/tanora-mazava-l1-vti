  ...
  return (
    <main style={styles.main}>
      <section style={styles.card}>
        ...
        <p style={styles.text}>Module économie stable. Tohizo mankany amin’ny Taniketsa Fandraharahana.</p>
        <div style={styles.actions}>
          <button style={styles.secondaryButton} onClick={onBack}>Miverina</button>
          <button style={styles.button} onClick={onNext}>Manaraka</button>
        </div>
...
function TaniketsaForm({ onBack }: any) {
  ...
  const totalScore = scores.reduce(
    (sum, s) => sum + s.tany + s.fiofanana + s.ezaka + s.tohana + s.economie,
    0
  );
    ...
    <main style={styles.main}>
      <section style={styles.card}>
        ...
        <div style={styles.scoreBox}>
          ...
          <strong>Total Score : </strong>{totalScore} / 275
        </div>
        ...
        {filieres.map((f, i) => {
          ...
          return (
            <div key={f.name} style={styles.block}>
              ...
              {[0, 1, 2].map((year) => {
                ...
                return (
                  <div key={year} style={styles.miniBox}>
                    ...
                    {f.type === "tantely" && (
                      <>
                        <input style={styles.input} type="number" placeholder="CA vinavinaina amin’ny tantely" onChange={(e) => {
                          const copy = [...tantelyCA]; copy[year] = Number(e.target.value); setTantelyCA(copy);
                        }} />
                        <input style={styles.input} type="number" placeholder="Dépenses vinavinaina amin’ny tantely" onChange={(e) => {
                          const copy = [...tantelyDep]; copy[year] = Number(e.target.value); setTantelyDep(copy);
                        }} />
                      </>
                    )}
              ...
              <h4 style={styles.sectionTitle}>E. Fanadihadiana ara-toekarena — 10 points</h4>
              <textarea style={styles.textarea} placeholder="Hazavao raha mitombina ny tetikasa: tany, fiofanana, ezaka, tohana, dépenses, CA, bénéfice." />
              <ScoreSelect label="Score faisabilité économique" max={10} onChange={(v: number) => updateScore(i, "economie", v)} />
