...
export default function HomePage() {
  ...
  if (screen === "identite") {
    return <IdentiteForm onBack={() => setScreen("home")} onNext={() => setScreen("spirituel")} />;
  }

  if (screen === "spirituel") {
    return <SpirituelForm onBack={() => setScreen("identite")} onNext={() => setScreen("vti")} />;
  }

  if (screen === "vti") {
    return <VtiForm onBack={() => setScreen("spirituel")} onNext={() => setScreen("economie")} />;
  }

  if (screen === "economie") {
    return <EconomieForm onBack={() => setScreen("vti")} onNext={() => setScreen("taniketsa")} />;
  }
  ...
  return (
    <main style={styles.main}>
      <section style={styles.card}>
        ...
        <p style={styles.text}>
          Module économie stable. Ny Taniketsa Fandraharahana no ampidirina eto ambany.
        </p>
...
function TaniketsaForm({ onBack }: any) {
  ...
  const updateTantelyCA = (year: number, value: number) => {
    const copy = [...tantelyCA];
    copy[year] = value;
    setTantelyCA(copy);
  };

  const updateTantelyDep = (year: number, value: number) => {
  ...
  const yearData = (i: number, year: number) => {
    ...
    const ca = f.name === "Tantely" ? tantelyCA[year] : n * f.caRef[year];
    const dep = f.name === "Tantely" ? tantelyDep[year] : n * f.depRef[year];
                    ...
                    {f.name === "Tantely" && (
                      <>
                        <input
                          ...
                          onChange={(e) => updateTantelyCA(year, Number(e.target.value))}
                        />
                        ...
                        <input
                          ...
                          onChange={(e) => updateTantelyDep(year, Number(e.target.value))}
                        />
                      </>
                    )}
